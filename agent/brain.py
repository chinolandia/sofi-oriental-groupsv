...
# Las API keys "identity-linked" de Anthropic exigen indicar en que workspace opera
# la peticion, con el header anthropic-workspace-id. Si tu key es una key normal de
# workspace, deja ANTHROPIC_WORKSPACE_ID vacio y no se manda nada.
...
def cargar_plantilla_base() -> str:
    """
    La parte reutilizable del system prompt (config/prompts.yaml -> "base").
    Lleva el marcador {bloque_tienda} que se reemplaza por la tienda detectada.
    """
    cfg = cargar_config_prompts()
    # "base" es lo nuevo (multi-tienda). "system_prompt" es el formato viejo: se
    # acepta como fallback para no romper si alguien edito el YAML a mano.
    return cfg.get("base") or cfg.get(
        "system_prompt", "Eres Sofi, asesora de ventas. Responde siempre en espanol."
    )


def construir_system_prompt(tienda_id: str | None, nombre_cliente: str | None = None) -> str:
    """
    Arma el system prompt final para esta conversacion:
    base + bloque de la tienda detectada (o bloque "sin tienda") + nombre del cliente.
    """
    base = cargar_plantilla_base()

    if tienda_id and tiendas.existe(tienda_id):
        bloque = tiendas.construir_bloque_tienda(tienda_id)
    else:
        bloque = tiendas.bloque_sin_tienda()

    prompt = base.replace("{bloque_tienda}", bloque)

    if nombre_cliente:
        prompt += (
            f"\n\n## Dato del cliente\n"
            f"El nombre del cliente en WhatsApp es: {nombre_cliente}. Usalo con "
            f"naturalidad. Si ya lo tenes, no vuelvas a pedirlo para el pedido."
        )
    return prompt
...
async def generar_respuesta(
    ...
    Args:
        ...
        tienda_id: la tienda del paraguas detectada para esta conversacion (o None)
        nombre_cliente: nombre del cliente en WhatsApp, si el proveedor lo entrego
    ...
    system_prompt = construir_system_prompt(tienda_id, nombre_cliente)
