...
class TiendaConversacion(Base):
    """
    A que tienda del paraguas (Oriental Group) pertenece cada conversacion.

    Con numero de WhatsApp compartido, la tienda se detecta por el anuncio de origen,
    y SOLO el primer mensaje trae ese dato. Aca se fija para que los mensajes
    siguientes de ese mismo cliente sigan hablando con la tienda correcta.
    """

    __tablename__ = "tienda_conversacion"

    telefono: Mapped[str] = mapped_column(String(50), primary_key=True)
    tienda_id: Mapped[str] = mapped_column(String(80))
    definido_en: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=ahora)
...
async def fijar_tienda(telefono: str, tienda_id: str):
    """
    Deja registrado que la conversacion con este numero pertenece a 'tienda_id'.
    Si ya habia una tienda distinta, la actualiza (el cliente pudo llegar desde
    un anuncio nuevo de otra tienda).
    """
    if not telefono or not tienda_id:
        return
    async with async_session() as session:
        fila = await session.get(TiendaConversacion, telefono)
        if fila is None:
            session.add(
                TiendaConversacion(telefono=telefono, tienda_id=tienda_id, definido_en=ahora())
            )
        elif fila.tienda_id != tienda_id:
            fila.tienda_id = tienda_id
            fila.definido_en = ahora()
        await session.commit()


async def tienda_de_conversacion(telefono: str) -> str | None:
    """Devuelve la tienda fijada para este numero, o None si no hay."""
    if not telefono:
        return None
    async with async_session() as session:
        fila = await session.get(TiendaConversacion, telefono)
        return fila.tienda_id if fila else None
