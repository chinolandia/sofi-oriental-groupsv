...
from agent.memory import (
    ...
    obtener_historial,
    tienda_de_conversacion,
)
from agent.providers import obtener_proveedor
from agent.providers.base import MensajeEntrante
from agent.tiendas import resolver_por_referral, tienda_por_defecto
...
async def procesar_mensaje(msg: MensajeEntrante):
    ...
    async with _candados[msg.telefono]:
        try:
            # ¿De que tienda del paraguas es esta conversacion?
            #   1. Si el mensaje trae "referral" (viene de un anuncio), se detecta y se fija.
            #   2. Si no, se recupera la tienda ya fijada para este numero.
            #   3. Si tampoco, se usa la tienda por defecto (config/business.yaml).
            tienda_id = None
            referral = msg.contexto.get("referral")
            if referral:
                tienda_id = resolver_por_referral(referral)
                if tienda_id:
                    await fijar_tienda(msg.telefono, tienda_id)
            if tienda_id is None:
                tienda_id = await tienda_de_conversacion(msg.telefono)
            if tienda_id is None:
                tienda_id = tienda_por_defecto()
            logger.info(f"Tienda de la conversacion con {msg.telefono}: {tienda_id or '(sin identificar)'}")

            nombre_cliente = msg.contexto.get("nombre_cliente")
            ...
            respuesta, es_respuesta_real = await generar_respuesta(
                msg.texto, historial, tienda_id, nombre_cliente
            )
